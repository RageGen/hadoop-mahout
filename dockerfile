...
RUN apt-get update && \
    apt-get install -y curl openjdk-8-jdk wget python3 python3-pip ssh tzdata nano
...
RUN wget https://archive.apache.org/dist/mahout/0.13.0/apache-mahout-distribution-${MAHOUT_VERSION}.tar.gz && \
    tar -xzvf apache-mahout-distribution-${MAHOUT_VERSION}.tar.gz && \
